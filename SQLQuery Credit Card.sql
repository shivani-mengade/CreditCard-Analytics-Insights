...
-- Revenue by crad category
SELECT Card_Category,
    ROUND(SUM(Total_Trans_Amt + Annual_Fees + Interest_Earned),2) AS Revenue
FROM c_details
GROUP BY Card_Category
...
SELECT
    CASE
        WHEN Avg_Utilization_Ratio < 0.3 THEN 'Low Usage'
        WHEN Avg_Utilization_Ratio BETWEEN 0.3 AND 0.7 THEN 'Medium Usage'
        ELSE 'High Usage'
...
-- Cumalitive Analysis
--Total revenue per month and running total of revenue over time.
